...
#define MASK GDK_BUTTON_PRESS_MASK | GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK
  ...
  if (bm.x < 0)
    bm.x += 1 + gdk_screen_get_width (defscrn) - attr.width;
  if (bm.y < 0)
    bm.y += 1 + gdk_screen_get_height (defscrn) - attr.height;
  attr.x = bm.x;
  attr.y = bm.y;
  attr.title = "hot-babe";
...
//  gdk_window_set_keep_below(bm.win, TRUE);
  ...
  gdk_pixbuf_render_pixmap_and_mask (bm.anim.pixbuf[bm.anim.samples - 1],
				     &pixmap, &mask, 127);
...
hotbabe_update (void)
{
  ...
  if (bm.threshold)
	...
	loadPercentage =
	  (loadPercentage - bm.threshold) * 256 / (256 - bm.threshold);
    }
  ...
  if (loadPercentage != old_percentage)
      ...
      for (i = 0; i < bm.anim.height * bm.anim.width; i++)
	{
	  ...
	  if (src1[3])
		...
		{
		  val =
		    ((guint) * (src2++)) * loadPercentage +
		    ((guint) * (src1++)) * (range - loadPercentage);
		  *(dest++) = val / range;
		  //*(dest++) = (val >> 6);  // bad hack!
		}
	      src1++;
      ...
      gdk_draw_rgb_image (pixmap, gc, 0, 0, bm.anim.width, bm.anim.height,
			  GDK_RGB_DITHER_NONE, bm.dest, 3 * bm.anim.width);
      gdk_window_set_back_pixmap (bm.win, pixmap, False);
  ...
  if (dir != NULL)
    {
	  ...
	  load_anim (&bm.anim, home) && load_anim (&bm.anim, dir))
	{
